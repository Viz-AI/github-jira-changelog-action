...
describe('parseRange', () => {
  test('parses symmetric <sha1>...<sha2>', () => {
    const range = (0, _cli.parseRange)('e9f569df...cd1f7cd6');
  ...
  test('parses <sha1>', () => {
    const range = (0, _cli.parseRange)('e9f569df');
    expect(range.from).toBe('e9f569df');
    expect(range.to).toBe('');
    expect(range.symmetric).toBe(false);
  });
  test('invalid pattern ...', () => {
    expect(() => (0, _cli.parseRange)('...')).toThrow(Error);
  });
  test('invalid pattern ..', () => {
    expect(() => (0, _cli.parseRange)('..')).toThrow(Error);
  });
  test('parses versions with decimal and 3 in between versions', () => {
    const range = (0, _cli.parseRange)('v1.23...v2.12');
