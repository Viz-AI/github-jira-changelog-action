...
class Jira {
   ...
   * @param {String} excluded_tickets - A comma separated list of ticket keys to exclude from the changelog.
   * @return {Object}
   */


  async generate(commitLogs, releaseVersion = null, excluded_tickets = null) {
    var logs = [];
    this.releaseVersions = [];

    try {
      const promises = commitLogs.map(commit => this.findJiraInCommit(commit, releaseVersion).then(log => {
        logs.push(log);
      }).catch(e => {
        console.error(e);
      }));
      ...
      await Promise.all(promises); // Get all Jira tickets (filter out duplicates by keying on ID)

      // exclude tickets
      if (excluded_tickets) {
        const excludedTickets = excluded_tickets.split(',');
        excludedTickets.forEach((ticket, index) => {
          excludedTickets[index] = ticket.trim();
        });
        logs.forEach(log => {
          log.tickets = log.tickets.filter(ticket => !excludedTickets.includes(ticket.key));
        });
      }
      ...
      // log the found tickets by key
      console.log('Found tickets (minus excluded):');
  ...
  async findJiraInCommit(commitLog) {
    ...
    const promises = [];
    const found = {}; // Search for jira ticket numbers in the commit text

    // use the summary and fulltext to find tickets
    const commitText = `${log.summary} ${log.fulltext}`;

    const ticketKeys = this.parseTicketsFromString(commitText);
    ticketKeys.forEach(key => {
      ...
      found[key] = true;
      promises.push(this.fetchJiraTicket(key).catch(() => {}) // ignore errors
      );
  ...
  async addTicketsToReleaseVersion(tickets, versionName) {
    ...
    this.releaseVersions = []; // Create version and add it to a ticket
  ...
  async getJiraIssue(ticketId) {
    ...
    return this.jira.findIssue(ticketId).then(origTicket => {
      ...
      return this.slack.findUser(ticket.fields.reporter.emailAddress, ticket.fields.reporter.displayName).then(slackUser => {
  ...
  parseTicketsFromString(str) {
    ...
    const regex = new RegExp(configPattern, 'g');
    ...
    while ((m = regex.exec(str)) !== null) {
       ...
       // add the ticket key to the list, uppercased, and no duplicates
        ticketKeys.push(m[1]);
        
    }

    // return the ticket keys, unique, non-null and uppercased
    return ticketKeys.map(t => t.toUpperCase()).filter((t, i, a) => a.indexOf(t) === i && t !== null);
  }
