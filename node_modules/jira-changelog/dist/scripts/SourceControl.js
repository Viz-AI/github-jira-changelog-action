...
class SourceControl {
  ...
  getCommitLogs(workspaceDir, range) {
    ...
    return new Promise((resolve, reject) => {
      ...
      workspace.log(opts, (err, response) => {
        ...
        const logs = this.consolodateCommitMessages(graph); // Add slack users to commit logs
  ...
  isRevert(log) {
    ...
    const match = oneLine.match(/^Revert ".*?This reverts commit ([0-9a-z]+)\.$/); // Is it a revert

    if (match) {
      const [, revertSha] = match; // If the summary has an even number of reverts listed, it has been unreverted
  ...
  simpleTopLevelGraph(logs) {
    const hashes = {};
    const graph = []; // First get the hashes from all commits and turn parents into an array
    // and decorate each log with the 'reverted' property

    const logObjs = logs.map(l => {
      const log = { ...l
      };
      const parents = log.parents.split(' '); // Create log graph
    ...
    function relatedLogs(logItem) {
      ...
      const related = [logItem.graph.prev, logItem.graph.parents];
