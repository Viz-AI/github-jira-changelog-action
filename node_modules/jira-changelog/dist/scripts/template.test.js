...
describe('filter reverts', () => {
  test('the revert commit is removed when the original commit is in the list', () => {
    const logs = [{
      ...
      revision: 11,
      reverted: 10
    }, // filter this revert commit out & keep revision 10
    {
      revision: 12
    }];
    const filtered = (0, _template.filterRevertedCommits)(logs);
  ...
  test('the revert commit is retained when original commit is not in list', () => {
    const logs = [{
      ...
      revision: 11,
      reverted: 5
    }, // keep revert commit because revision 5 isn't in list
    {
      revision: 12
    }];
    const filtered = (0, _template.filterRevertedCommits)(logs);
...
describe('Mark tickets as reverted', () => {
  test('out of order commits', () => {
    const tickets = [{
      key: 'ENG-123',
      commits: [// Not in date order
      {
        date: '2020-02-02T16:02:36-08:00',
        reverted: null
      }, {
    ...
    (0, _template.decorateTicketReverts)(tickets);
...
test('Get all ticket reporters', () => {
  ...
  const reporters = (0, _template.getTicketReporters)(tickets);
...
describe('Get pending tickets', () => {
  test('single approval status in config', () => {
    ...
    const groups = (0, _template.groupTicketsByStatus)(config, tickets);
  ...
  test('no approval status in config', () => {
    ...
    let groups = (0, _template.groupTicketsByStatus)(config, tickets);
    expect(groups.pending.map(t => t.key)).toEqual(['ENG-1234', 'ENG-234']);
    expect(groups.approved.length).toBe(0); // Empty list

    config.jira.approvalStatus = [];
    groups = (0, _template.groupTicketsByStatus)(config, tickets);
    expect(groups.pending.map(t => t.key)).toEqual(['ENG-1234', 'ENG-234']);
...
test('transform commit logs into template data', () => {
  ...
  const transformed = (0, _template.transformCommitLogs)(config, commitLogs);
  const {
    commits,
    tickets
  } = transformed;
...
test('hideEmptyBlocks with false', async () => {
  const config = { ...(0, _Config.getDefaultConfig)(),
    hideEmptyBlocks: false
  };
  const templateData = await (0, _template.generateTemplateData)(config, [], []);
  const templateRendered = (0, _template.renderTemplate)(config, templateData);
