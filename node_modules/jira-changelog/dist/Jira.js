    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        ...
        .then(() => {
          ...
          // exclude all tickets in ticketsList that already have a fixVersion
          // ticketsList = ticketsList.filter(ticket => {
          //   return !ticket.fields.fixVersions || !ticket.fields.fixVersions.length;
          // });
          console.log(`Found ${ticketsList.length} tickets with no fix version set`);
          if (ticketsList.length && releaseVersion) {
            return _this.addTicketsToReleaseVersion(ticketsList, releaseVersion).then(() => logs);
          }
          // exclude tickets that already have a fixVersion from the logs as well
          // so they don't show up in the changelog
          // logs = logs.map(log => {
          //   log.tickets = log.tickets.filter(ticket => {
          //     return !ticket.fields.fixVersions || !ticket.fields.fixVersions.length;
          //   });
          //   return log;
          // });
