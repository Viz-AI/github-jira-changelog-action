      ...
      try {
        ...
        .then(() => {
          ...
          if (ticketsList.length && releaseVersion) {
            return _this.setFixVersionFortickets(ticketsList, releaseVersion).then(() => logs);
          }
    ...
    return _asyncToGenerator(function* () {
      const promises = tickets.map(ticket => {
        return _this5.jira.updateIssue(ticket.id, {
          ...
          console.warn(`Failed to set fix version ${versionName} for ${ticket.id}`);
          throw new Error(`Failed to set fix version ${versionName} for ${ticket.id}`);
        });
      });
