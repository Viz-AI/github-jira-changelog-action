...
class Jira {
      ...
      try {
        ...
        .then(() => {
          ...
          if (ticketsList.length && releaseVersion) {
            return _this.createReleaseVersion(releaseVersion, ticketsList).then(() => logs);
          }
  ...
  getTickets(log) {
    ...
    return log.summary.match(searchPattern) || [];
  }

  setFixVersionForTickets(tickets, versionName) {
    var _this5 = this;
    console.log(`Setting fix version for ${tickets.length} tickets to ${versionName}`);
    return _asyncToGenerator(function* () {
      const promises = tickets.map(ticket => {
        return _this5.jira.updateIssue(ticket.id, {
          fields: {
            fixVersions: [{
              name: versionName
            }]
  ...
  // creates a release version for 'Software Group' if not already exists
  // and sets the fix version for all tickets in the release
  createReleaseVersion(versionName, tickets) {
    var _this6 = this;
    return _asyncToGenerator(function* () {
      const projectKey = 'VSW';
      const version = yield _this6.createProjectVersion(versionName, projectKey);
      yield _this6.setFixVersionForTickets(tickets, version.name);
    })();
  }
